  var Entry = function(config) {
    var checkNext     = true, 
        elemsToShow   = [],
        windowScrollY = 0,
        offset        = (config && config.offset) ? config.offset : 200,
        duration      = (config && config.duration) ? config.duration : '.5',
        transition    = 'all ' + duration + 's';
    this.elemsToShow  = elemsToShow;
    ...
    function _initElemsToShow() {
      ...
      doms.forEach(function(elem){
        ...
        elemsToShow.push({
          dom: elem,
          position: elem.getClientRects()[0].top + windowScrollY + offset - yOffset,
          yOffset: yOffset
        })
      });
    }
    function _handleScroll() {
      windowScrollY = window.scrollY;
      checkNext = true;
      for(i = elemsToShow.length - 1; i >= 0; i--) {
    ...
    function _handleResize() {
      ...
      elemsToShow.forEach(function(elem){
        var newPos = elem.dom.getClientRects()[0].top + windowScrollY + offset - elem.yOffset;
